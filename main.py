...
from bokeh.core.property.dataspec import TextBaselineSpec
from bokeh.core.property.vectorization import Value
from bokeh.events import Pan, RangesUpdate, DocumentReady
...
from bokeh.models import Select, DatePicker, TextInput, Button, TableColumn, DataTable, CrosshairTool, HoverTool, \
    SaveTool, RadioGroup, Plot, Text, Label, DataRange1d, LinearScale
        ...
        try:
            ...
            if funct == "wordcloud":
                ...
                if os.path.exists(font_path):
                    try:
                        wordcloud_data = tweets.get_wordcloud(width=1000, height=800, font_path=font_path,
                                                              color_function=color_function)
                    except OSError:
                        self.show_error_message("Wystąpił błąd przy wczytywaniu czcionki. "
                                                "Chmura słów może zawierać błędy.")
                        wordcloud_data = tweets.get_wordcloud(width=1000, height=800, color_function=color_function)
                else:
                ...
                word_freq, wordcloud, (width, height) = wordcloud_data
                initial_scale = float(height) / float(width) if width > height else 1
                ...
                def format_font_size(size):
                    if not math.isfinite(size):
                        return "1px"
                    base = f"{size:.8f}".rstrip("0")
                    if base.endswith("."):
                        return f"{base}0px"
                    return f"{base}px"
                ...
                text_size_raw = [v[1] for v in wordcloud]
                text_size = [format_font_size(v * initial_scale) for v in text_size_raw]
                ...
                text_baseline = ["top"] * len(wordcloud) # should be "hanging" but bokeh interprets it as "alphabetic"
                ...
                #update text scale when zooming in (filters out position change events)
                old_scale = [-1.0]
                def update_scale(event):
                    scale = fig.height / (fig.y_range.end - fig.y_range.start)
                    if math.isfinite(scale) and not math.isclose(scale, old_scale[0], rel_tol=0.1):
                        source.data['text_size'] = [f"{v * scale:.5f}px" for v in text_size_raw]
                        old_scale[0] = scale
                fig.on_event(RangesUpdate, update_scale)
